...
import { ChevronRight, ArrowRight, Calendar, MapPin, User, ArrowDown, UserPlus, Handshake, Ticket } from 'lucide-react';
...
export default function Hero({ onOpenRegister, onOpenSponsor, onOpenVolunteer, onGetPass, onExploreEvent }: HeroProps) {
  ...
  return (
    <section 
      id="hero" 
      className="relative min-h-screen flex flex-col items-center justify-center px-4 md:px-8 pt-16 sm:pt-20 md:pt-28 pb-0"
    >
      ...
      {/* Background GIF */}
      <div 
        className="absolute -top-[280px] sm:-top-32 left-1/2 -translate-x-1/2 w-full max-w-[1920px] h-[1100px] z-0 pointer-events-none opacity-50 mix-blend-screen blur-[4px]"
        style={{
      ...
      >
        <DotLottieReact
          ...
          loop
          autoplay
          className="w-full h-full object-cover scale-[1.8] sm:scale-100"
        />
      </div>
      ...
      <div className="relative z-20 max-w-7xl mx-auto w-full flex flex-col items-center justify-center">
        ...
        <div className="flex flex-col justify-center items-center text-center">
          ...
          <motion.div
            ...
            className="mb-10 sm:mb-10 flex justify-center"
          >
            <div className="relative inline-block text-center px-4">
              <p className="text-[18px] sm:text-[20px] md:text-[24px] text-slate-200 font-jura font-medium tracking-wide">
          ...
          <motion.div
            ...
            className="w-full max-w-[95%] sm:max-w-4xl mx-auto mb-4 relative px-1 sm:px-0"
          >
            <div className="w-full rounded-[24px] sm:rounded-[32px] py-3 px-4 sm:p-8 bg-gradient-to-r from-brand-pink via-brand-purple to-brand-blue bg-[length:200%_auto] animate-[gradient_6s_ease-in-out_infinite] flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-10 shadow-[0_8px_32px_rgba(120,45,255,0.4)]">
              
              <div className="text-center sm:text-left text-white shrink-0 mt-1 sm:mt-0">
                <h4 className="font-jura font-bold text-lg sm:text-xl drop-shadow-sm whitespace-nowrap">Join Techcon '26</h4>
              </div>
              
              <div className="flex flex-row items-center justify-between sm:justify-center gap-3 sm:gap-4 w-full sm:w-auto mt-2 sm:mt-0 px-1 sm:px-0">
                <button
                  onClick={onOpenRegister}
                  className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-1.5 px-2 sm:px-8 sm:py-3.5 bg-brand-dark/90 backdrop-blur-sm text-slate-200 font-sans font-bold text-[10px] sm:text-sm rounded-[14px] sm:rounded-full flex-1 sm:flex-none aspect-square sm:aspect-auto shrink-0 border border-slate-700/50 hover:bg-brand-purple hover:text-white hover:border-brand-purple transition-colors duration-300 cursor-pointer"
                >
                  <UserPlus className="w-6 h-6 sm:hidden mb-0.5" strokeWidth={1.5} />
                  <span className="text-center sm:text-left leading-tight sm:leading-normal">
                    <span className="sm:hidden text-[10px]">Register<br/>Now</span>
                ...
                <button
                  ...
                  className="flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 py-1.5 px-2 sm:px-6 sm:py-3.5 bg-brand-dark/90 backdrop-blur-sm text-brand-pink font-sans font-bold text-[10px] sm:text-sm rounded-[14px] sm:rounded-full flex-1 sm:flex-none aspect-square sm:aspect-auto shrink-0 border border-brand-pink/50 hover:bg-brand-pink hover:text-white transition-colors duration-300"
                >
                  <Handshake className="w-6 h-6 sm:hidden mb-0.5" strokeWidth={1.5} />
                  <span className="text-center sm:text-left leading-tight sm:leading-normal">
      ...
      <div className="relative z-20 w-full max-w-[500px] sm:max-w-[600px] lg:max-w-[800px] aspect-video flex items-center justify-center mt-2 lg:mt-4 -mb-8 lg:-mb-16 mix-blend-screen pointer-events-none scale-[1.15] sm:scale-100 origin-top translate-x-1.5 sm:translate-x-0">
        <HeroVideos />
      </div>
    </section>
  );
...
function HeroVideos() {
  ...
  useEffect(() => {
    const updateClip = () => {
      setClipInset(window.innerWidth < 768 ? 'inset(0 70px)' : 'inset(0 115px)');
    };
    updateClip();
