...
export default function RetrievePassForm({ onSuccess, onCancel, onOpenRegister }: RetrievePassFormProps) {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      try {
        const reg = await fetchPass(nameQuery, phone);
        if (reg) {
          onSuccess(reg);
        }
      } catch (err: any) {
        setErrorMsg(err.message || "No registration found with this Name or Phone Number. Please check your spelling or register anew.");
      }
    } catch (err) {
