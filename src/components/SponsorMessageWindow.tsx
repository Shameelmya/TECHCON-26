...
export default function SponsorMessageWindow({ isOpen, onClose, initialPlan }: SponsorMessageWindowProps) {
  ...
  const validate = () => {
    ...
    // Extract last 10 digits to validate Indian mobile numbers easily
        ...
        >
          ...
          >
              ...
              <div className="space-y-5">
                <div className="flex flex-col gap-2">
                  ...
                  <input 
                    ...
                    className={`w-full bg-slate-50 border-2 ${errors.companyName ? 'border-red-400' : 'border-slate-100'} rounded-2xl px-5 py-4 text-sm text-slate-900 focus:border-brand-purple focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium`}
                    placeholder="Enter your organization name"
                  />
                  {errors.companyName && <span className="text-red-500 text-[10px] ml-1">{errors.companyName}</span>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input 
                    ...
                    className={`w-full bg-slate-50 border-2 ${errors.contactPerson ? 'border-red-400' : 'border-slate-100'} rounded-2xl px-5 py-4 text-sm text-slate-900 focus:border-brand-purple focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium`}
                    placeholder="John Doe"
                  />
                  {errors.contactPerson && <span className="text-red-500 text-[10px] ml-1">{errors.contactPerson}</span>}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 border-2 ${errors.phone ? 'border-red-400' : 'border-slate-100'} rounded-2xl px-5 py-4 text-sm text-slate-900 focus:border-brand-purple focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium`}
                      placeholder="+91..."
                    />
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 border-2 ${errors.email ? 'border-red-400' : 'border-slate-100'} rounded-2xl px-5 py-4 text-sm text-slate-900 focus:border-brand-purple focus:bg-white outline-none transition-all placeholder:text-slate-400 font-medium`}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 shrink-0 relative z-10">
              <button 
                onClick={handleSend}
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-lg hover:shadow-emerald-500/30 hover:-translate-y-0.5"
              >
                <Send size={18} />
