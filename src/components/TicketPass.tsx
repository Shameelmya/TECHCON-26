...
import { Download, Share2, MapPin, Calendar, Compass, User, Printer, Scissors } from 'lucide-react';
...
export default function TicketPass({ registration, onBackToHome }: TicketPassProps) {
  ...
  useEffect(() => {
    ...
    // We will generate the ticket image on-demand when the user clicks download/share.
    // This avoids html2canvas getting stuck during mount.
  ...
  const handleSavePNG = async () => {
    ...
    try {
      ...
      const url = await toPng(ticketRef.current, { pixelRatio: 2, skipFonts: false });
      ...
      alert(`Failed to download image: ${error?.message || error}. Please try again or take a screenshot.`);
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const handleShareWhatsApp = async () => {
    ...
    try {
      ...
      // Async generation of image usually causes the browser to drop the "user gesture" token.
      // We will try to share, and if the browser blocks it, gracefully fallback to showing the image directly.
      const blob = await toBlob(ticketRef.current, { pixelRatio: 2, skipFonts: false });
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          ...
          // If user gesture was lost or user canceled, fallback to showing image overlay
          console.warn("Share failed (likely gesture lost), falling back to overlay", shareError);
          const url = await toPng(ticketRef.current, { pixelRatio: 2, skipFonts: false });
      ...
      alert(`Failed to generate image: ${error?.message || error}. Please try again or take a screenshot.`);
    } finally {
      setIsGenerating(false);
    }
  };
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-5 mb-5 select-none pt-3">
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  <img src="/loading-logo.png" alt="Logo" className="h-6 sm:h-7 object-contain" />
            ...
            <div className="space-y-5 mb-5 text-left relative z-10">
              <div>
                ...
                <span className="text-2xl font-bold font-sans tracking-tight text-slate-900 block truncate uppercase leading-none">
                  {registration.fullName}
                </span>
                <span className="text-sm font-semibold font-orbitron text-brand-pink block truncate uppercase mt-1.5">
                  {registration.occupation}
                </span>
              </div>

              {/* Details Box */}
              <div className="bg-slate-50/80 border border-slate-100 p-3 rounded-xl space-y-2.5 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3 text-[11px] font-mono">
                  <div>
                    <span className="text-[9px] text-slate-400 block mb-0.5">AGE</span>
                    <span className="font-bold text-slate-700">{registration.age}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] text-slate-400 block mb-0.5">DISTRICT</span>
                    <span className="font-bold text-slate-700 truncate">{registration.district}</span>
                  </div>
                </div>
                
                {registration.place && (
                  <div className="text-[11px] font-mono pt-0.5">
                    <span className="text-[9px] text-slate-400 block mb-0.5">PLACE</span>
                    <span className="font-bold text-slate-700 truncate">{registration.place}</span>
                  </div>
                )}
              </div>
            ...
            <div className="relative flex items-center justify-center my-6 py-2 opacity-60">
              <div className="absolute left-0 right-0 w-full border-t-[2px] border-dashed border-slate-300" />
              <Scissors className="absolute left-4 text-slate-400 rotate-180 bg-white px-1" size={24} />
            </div>
            ...
            {/* Barcode */}
            <div className="mt-4 flex flex-col items-center gap-1 opacity-80 select-none">
              <div 
                className="w-full max-w-[200px] h-8" 
                style={{ 
                  background: 'repeating-linear-gradient(90deg, #1e293b 0px, #1e293b 2px, transparent 2px, transparent 4px, #1e293b 4px, #1e293b 5px, transparent 5px, transparent 8px, #1e293b 8px, #1e293b 12px, transparent 12px, transparent 14px)'
                }}
              />
              <span className="text-[8px] font-mono tracking-[0.3em] text-slate-500">{registration.ticketNumber}</span>
            </div>
          ...
          <div className="bg-slate-50 border border-slate-150/80 p-6 rounded-3xl space-y-4">
            ...
            <div className="flex flex-row gap-3">
              ...
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center p-3 sm:p-4 bg-white hover:bg-slate-50 border hover:border-purple-300 rounded-2xl font-sans transition-all duration-300 shadow-sm gap-2 active:scale-95 ${
                  isGenerating ? 'opacity-50 cursor-not-allowed border-slate-200 text-slate-400' : 'border-slate-200 text-slate-800'
                }`}
              >
                <Download size={20} className={isGenerating ? 'text-slate-400' : 'text-purple-600'} />
                <span className="font-semibold text-[10px] sm:text-xs">{isGenerating ? 'Processing...' : 'Download'}</span>
              </button>
              ...
              <button
                ...
                disabled={isGenerating}
                className={`flex-1 flex flex-col items-center justify-center p-3 sm:p-4 bg-white hover:bg-emerald-50 border hover:border-emerald-300 rounded-2xl font-sans transition-all duration-300 shadow-sm gap-2 active:scale-95 ${
                  isGenerating ? 'opacity-50 cursor-not-allowed border-slate-200 text-slate-400' : 'border-slate-200 text-slate-800'
                }`}
              >
                <svg viewBox="0 0 24 24" className={`w-5 h-5 fill-current ${isGenerating ? 'text-slate-400' : 'text-emerald-500'}`} xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="font-semibold text-[10px] sm:text-xs">{isGenerating ? 'Processing...' : 'Share'}</span>
              </button>
            </div>
          </div>
